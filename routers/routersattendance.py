...
def show_attendance():
    try:
        ...
        children = Child.query.filter_by(is_active=True).order_by(
...
def add_attendance():
    ...
    try:
        ...
        child_name = f'{child.first_name} {child.last_name}' if child else 'الطفل'
...
def bulk_attendance():
    ...
    try:
        ...
        if error_count > 0:
            flash(f'فشل في تسجيل {error_count} من الأطفال لأن لديهم سجلات مسبقة.', 'warning')
...
def attendance_report():
    ...
    try:
        ...
        if class_id:
            query = query.join(Child).filter(Child.class_id == int(class_id))
